...
- A statusline payload without `rate_limits` (a signed-out session, or one before its first API response) leaves the file untouched instead of nulling every stored window.
