...
6. Task Bar separately reads Claude Desktop's local Home conversation-list cache from the claude.ai IndexedDB blob directory. Home rows use cached `updated_at` as recent/unread activity and never infer a running state because the persisted list does not expose generation status.
