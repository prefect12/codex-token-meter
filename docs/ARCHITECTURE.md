...
- `Sources/CodexTokenMeter/OpenCodeTokenScanner.swift`: read-only scanner for the local OpenCode / OpenChamber SQLite session database. Assistant message rows carry per-message token totals (summed directly, not delta-counted), and subagent sessions are included. Its reports merge into the API partition everywhere that partition is built.
...
- `Sources/CodexPetBar/OpenCodeActivityReader.swift`: Task Bar's read-only OpenCode / OpenChamber SQLite session reader. It uses persisted role/completion data for activity state, adds text previews and message turns, and expires non-pinned stopped sessions after a short diagnostic window.
- `Sources/CodexPetBar/TaskCostEstimator.swift`: Task Bar's per-task API-equivalent cost estimator. It reuses the shared built-in model rates and reads AI Token Meter's existing manual/OpenRouter pricing caches without fetching or writing data.
- `Sources/CodexTokenMeter/BuiltInAPIModelRates.swift`: pure built-in API-equivalent model rates compiled into both AI Token Meter and Task Bar so the two apps do not drift on fallback pricing.
...
- `Sources/CodexTokenMeter/ClaudeModelRoutingStore.swift`: read/write support for Claude Code user defaults and private project overrides. It preserves unrelated JSON settings, updates `model`, `effortLevel`, `autoCompactWindow`, and the early-compaction percentage in `env`, and writes project overrides to `.claude/settings.local.json` so shared repository configuration is not changed. Linked Git worktrees resolve that local file to the main checkout root, matching Claude Code v2.1.211+. For the current Opus alias, global effort is also saved under `modelSettings.claude-opus-5-5` because that model ignores the legacy user-level `effortLevel`; project-local `effortLevel` still applies. The compaction window changes only the auto-compaction calculation; it does not enlarge a model's actual context capacity.
...
2. It parses `session_meta.model_provider`, `turn_context.model`, and cumulative `token_count` rows. Non-subscription providers (anything other than the known Codex subscription providers) and provider-qualified model IDs such as `anthropic/...` or `google/...` are classified as API-billed usage. Codex reports exclude those events, API reports include them, and All merges the partitions exactly once. Model-name fallbacks exist only for legacy DeepSeek rows that predate provider attribution.
...
4. `OpenCodeTokenScanner` reads OpenCode / OpenChamber assistant messages from the local SQLite store. Message tokens are per-message counts mapped with Claude-style inclusive input (`input + cache.read + cache.write`). The report merges into the API partition alongside provider-attributed rollout events and the optional external import file.
5. Day/week/month Codex windows can reuse day-level parsed-rollout caches; rolling `24h` windows stay event-based. Claude and OpenCode scans stay file/event based.
6. Task Bar separately reads Claude Desktop's local Home conversation-list cache from the claude.ai IndexedDB blob directory and recent OpenCode sessions from the local SQLite store. The user chooses one Codex home directory, one Claude root directory, and one OpenCode directory in Task Bar's local settings; the readers never write to those sources. Home rows use cached `updated_at` as recent/unread activity and never infer a running state because the persisted list does not expose generation status. OpenCode rows rely only on persisted role and completion state.
   When a row has a recognized model plus detailed input/cache/output counters, its optional Hover cost field shows an API-equivalent USD estimate. Missing counters or unknown prices suppress the row instead of guessing. This is not a subscription charge.
7. `LiveRateLimitReader` reads `https://chatgpt.com/backend-api/wham/usage` directly with the existing local ChatGPT credentials. The response is cached in memory, successful snapshots are persisted, and failures use 1/5/15-minute backoff without launching Codex processes.
8. `ClaudeOAuthUsageRefresher` refreshes Claude quota through the read-only OAuth usage endpoint during the normal live-refresh cycle. The `cedar_ember=1` usage extension can also return Claude Code reset grants; these are held in memory for at most ten minutes and shown only when the response explicitly marks the account eligible and a grant remains valid. Missing or surface-ineligible data is unknown, not zero. The extension is undocumented and requires a recent Claude Code user-agent prefix, so the app identifies itself in the suffix and falls back to the plain usage URL if the extension is rejected. It never calls a reset-consumption endpoint. Overview uses the same expiry/countdown renderer and one-second redraw timer for Codex and Claude reset grants. The All overview places the two platforms side by side, and the overview omits the model-summary card (full model details remain in the Models section). App launch checks only legacy ACL metadata and silently reuses an existing persistent `/usr/bin/security` grant; it never requests Keychain access automatically. A new or revoked grant can be requested only through the explicit `--grant-claude-keychain` command, where choosing **Always Allow** persists Apple's stable system tool in the legacy decrypt ACL instead of authorizing each newly built AI Token Meter CDHash. Before every background read, the app checks the ACL and login-keychain lock state without decrypting the item; missing persistent authorization or a locked keychain silently falls back to statusline/cache data instead of launching a password dialog. Keychain credential reads and rotated-token writes both run through the stable `/usr/bin/security` executable, so app rebuilds do not accumulate obsolete trusted-application entries. Writes use `security -i` over stdin to keep the JSON credential out of process arguments and avoid the 128-byte limit of the direct interactive `-w` prompt. Rotated OAuth credentials are written back to their original shared source so Claude Code and AI Token Meter stay on the same refresh-token version. File-based credentials at `~/.claude/.credentials.json` are atomically written with mode `0600`. `ClaudeStatuslineStore` also captures optional Claude Code statusline `rate_limits` via `--claude-statusline` and exposes them as a local `LiveRateLimit` with `id=claude`.
9. `AccountUsageReader` reads `https://chatgpt.com/backend-api/wham/profiles/me` when Profile API totals are enabled and falls back to its last persisted snapshot when unavailable. Reset-credit details use the corresponding read-only `wham` endpoint and cache.
10. `CodexServiceStatusReader` reads `https://status.openai.com/api/v2/summary.json` for Codex component status.
11. `OpenRouterPricingCatalog` reads the public `https://openrouter.ai/api/v1/models` catalog at most once per day and persists only model identifiers and token rates. `AppDelegate` merges subscription Codex scans, Claude scans, provider-attributed API scans, optional external `api-usage.json`, Profile API totals, live quota limits, service status, and cost reference reports into `DashboardState`. The menu dashboard can seed this state from the last aggregate dashboard cache and last successful live quota cache while fresh scans run.
12. `DashboardView` renders the menu popover; `UsageDetailsView` renders details pages directly with AppKit drawing.
...
- Unknown or unlabeled models remain unpriced; price coverage must expose the gap rather than silently applying a default model.
...
~/.claude/projects
~/.local/share/opencode/opencode.db (read-only; OpenCode / OpenChamber)
...
Claude project overrides are written to `.claude/settings.local.json` (at the
main checkout root when the selected path is a linked Git worktree); unrelated
