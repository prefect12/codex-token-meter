...
6. `ClaudeOAuthUsageRefresher` refreshes Claude quota through the read-only OAuth usage endpoint during the normal live-refresh cycle. Credential reads and writes are non-interactive; if Keychain access is not already authorized, the app keeps statusline/cache data and never launches Claude Code or prompts for a password. `ClaudeStatuslineStore` also captures optional Claude Code statusline `rate_limits` via `--claude-statusline` and exposes them as a local `LiveRateLimit` with `id=claude`.
