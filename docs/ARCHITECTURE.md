...
6. `ClaudeOAuthUsageRefresher` refreshes Claude quota through the read-only OAuth usage endpoint during the normal live-refresh cycle. On the first app launch without file-based Claude credentials, AI Token Meter requests `Claude Code-credentials` once; choosing **Always Allow** persists access in the macOS Keychain ACL for the app's stable signing requirement. Every background process disables legacy Keychain interaction before reading, so missing or non-persistent authorization silently falls back to statusline/cache data instead of prompting again. Keychain credentials remain owned and refreshed by Claude Code; file-based credentials at `~/.claude/.credentials.json` may be refreshed and atomically written back. `ClaudeStatuslineStore` also captures optional Claude Code statusline `rate_limits` via `--claude-statusline` and exposes them as a local `LiveRateLimit` with `id=claude`.
