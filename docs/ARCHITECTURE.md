...
- `Sources/CodexTokenMeter/DetailsWindow.swift`: overview, model, calendar, storage, diagnostics, settings, and about pages. Cost-page drawing code remains in place for the underlying estimates, but the page is currently hidden from details-window navigation. The storage page scans lazily on first open via `AppDelegate` and only offers Reveal-in-Finder / copy-path actions, never deletion.
