...
- `Sources/CodexTokenMeter/CostEstimation.swift`: weekly reset observations, `QuotaCycleStore` per-reset-cycle usage history (5h and weekly windows, keyed by `resetsAt` with drift tolerance, backfilled once from legacy calendar-week snapshots), historical quota-value estimates, API-equivalent allocation helpers.
...
- `Sources/CodexTokenMeter/DetailsWindow.swift`: overview, model, calendar, quota-cycle, storage, diagnostics, settings, and about pages. The former money-focused costs page was replaced by the quota-cycle page (`drawQuotaCyclesPage`), which shows the live 5h/weekly windows plus per-cycle peak usage from `QuotaCycleStore`; legacy cost-page drawing code remains only for the estimate helpers reused elsewhere. The storage page scans lazily on first open via `AppDelegate` and only offers Reveal-in-Finder / copy-path actions, never deletion.
