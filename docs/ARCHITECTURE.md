...
6. `ClaudeOAuthUsageRefresher` refreshes Claude quota through the read-only OAuth usage endpoint during the normal live-refresh cycle only when Claude exposes file-based credentials at `~/.claude/.credentials.json`. AI Token Meter never reads or modifies `Claude Code-credentials` in Keychain. If a usable file token is unavailable, the app keeps statusline/cache data and never launches Claude Code or prompts for a password. File-based Claude credentials may be refreshed and atomically written back. `ClaudeStatuslineStore` also captures optional Claude Code statusline `rate_limits` via `--claude-statusline` and exposes them as a local `LiveRateLimit` with `id=claude`.
