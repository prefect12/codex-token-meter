...
7. `ClaudeOAuthUsageRefresher` refreshes Claude quota through the read-only OAuth usage endpoint during the normal live-refresh cycle. On the first app launch without file-based Claude credentials, AI Token Meter requests `Claude Code-credentials` once through Apple's stable `/usr/bin/security` executable; choosing **Always Allow** persists that system tool in the legacy decrypt ACL instead of authorizing each newly built AI Token Meter CDHash. Before every background read, the app checks the ACL and login-keychain lock state without decrypting the item; missing persistent authorization or a locked keychain silently falls back to statusline/cache data instead of launching a password dialog. Rotated OAuth credentials are written back to their original shared source so Claude Code and AI Token Meter stay on the same refresh-token version. File-based credentials at `~/.claude/.credentials.json` are atomically written with mode `0600`. `ClaudeStatuslineStore` also captures optional Claude Code statusline `rate_limits` via `--claude-statusline` and exposes them as a local `LiveRateLimit` with `id=claude`.
