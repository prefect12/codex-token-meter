...
- 详情窗口包含概览、日历、思考分析、模型、空间、设置、诊断和关于页面。
...
- **详情页缓存**：详情窗口会把 365 天总览、日历、金额、模型和思考分析所需的聚合快照缓存到本地 `details-snapshot-cache.json`。打开详情页时先显示上次快照，再后台重算并替换；缓存会移除 top session 路径和仓库真实路径，只保留展示名与统计值。
...
- `0.2.19` 为模型页新增 7/30/90 天快捷范围和自定义起止日期，补充会话、轮次、用量记录及周额度占比，并将默认模型配置扩展到 Claude；配置在外部变化时会自动同步。Task Bar `0.1.20` 修复新版 Codex Desktop 计划解析和计划固定按钮显示。
- `0.2.18` 新增全局与项目级 Codex 默认模型、思考强度配置，并用“跟随全局”复选框明确继承状态；同时恢复模型筛选、补充额度悬浮详情并修复仪表盘刷新与 Claude Keychain 授权持久化。Task Bar `0.1.19` 修复应用重启后计划信息丢失。
- `0.2.17` 为 Codex 与 Claude 周额度增加剩余节奏标记，提升详情页滚动与后台扫描性能，修复 Claude Keychain 自动提示、子智能体模型归属和 `claude-opus-5` API 等价价格；Task Bar `0.1.18` 新增计划进度悬浮，并修复 SQLite 进程累积、历史子任务重现和悬浮面板定位。
